...
public class ConfigUtil {
	...
	private ConfigUtil(File file){
		try{ // 지정된 값이 없을 경우 Resource폴더로 지정된 config.properties파일의 내용을 불러온다
			if(file==null || file.equals("") || file.length() == 0){
				String filename = ConfigUtil.class.getClassLoader().getResource("config.properties").getPath();
	 ...
	 */
	public static final ConfigUtil config = new ConfigUtil(null);
